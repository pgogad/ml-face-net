...
def compute_facial_encodings(sess, images_placeholder, embeddings, phase_train_placeholder, image_size,
    ...
    for i in range(nrof_batches):
        ...
        print('Completed %d/%d' % (i + 1, int(nrof_images / batch_size) + 1))
...
OUT_PUT = os.path.join(BASE_DIR, 'data', 'cluster')
...
def parse_args():
    ...
    parser.add_argument('--model_dir', type=str, help='model dir', default=MODEL_DIR)
    parser.add_argument('--batch_size', type=int, help='batch size', default=500)
    parser.add_argument('--input', type=str, help='Input dir of images', default=INPUT)
    parser.add_argument('--output', type=str, help='Output dir of clusters', default=OUT_PUT)
